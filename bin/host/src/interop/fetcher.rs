    ...
    async fn prefetch(&self, hint: &str) -> Result<()> {
        ...
        match hint_type {
            ...
            HintType::AgreedPreState => {
                ...
                if hash != keccak256(self.cfg.agreed_l2_pre_state.as_ref()) {
                ...
                kv_write_lock.set(
                    PreimageKey::new(*hash, PreimageKeyType::Keccak256).into(),
                    self.cfg.agreed_l2_pre_state.clone().into(),
                )?;
            }
            HintType::L2OutputRoot => {
                ...
                let hash = B256::from_slice(&hint_data.as_ref()[0..32]);
                let chain_id = u64::from_be_bytes(
                ...
                let pre = PreState::decode(&mut self.cfg.agreed_l2_pre_state.as_ref())
                    .map_err(|e| anyhow!("Failed to decode pre-state: {e}"))?;
...
#[async_trait]
impl<KV> HintRouter for InteropFetcher<KV>
where
