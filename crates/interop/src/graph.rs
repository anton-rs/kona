...
/// The dependency graph represents a set of blocks at a given timestamp and the interop dependencies between them.
///
...
pub struct DependencyGraph<P> {
    ...
    ///
    /// These are derived from the transactions within the blocks. If [None], the graph has yet to be built.
...
where
    P: InteropProvider,
{
    /// Creates a new dependency graph from a list of blocks (with chain IDs) and an [InteropProvider].
    pub fn new(blocks: &[(u64, Sealed<Header>)], provider: P) -> Self {
        Self { nodes: blocks.to_vec(), edges: Some(Vec::new()), provider }
    }

    /// Adds a block to the dependency graph. If [Self::edges] is [Some], the edges are discarded, and the graph must
    /// be rebuilt.
    ...
    /// Derives the edges from the blocks within the graph by scanning all transactions within the blocks and searching
    /// for [ExecutingMessage]s.
    async fn derive(&mut self) -> Result<(), ()> {
        ...
        for (chain_id, block_header) in self.nodes.iter() {
            let receipts = self.provider.block_receipts(block_header.hash_slow()).await;
