...
macro_rules! multiplexed_stage {
    (
        ...
        stages: {
            $($stage_name:ident$(<$stage_generic:tt: $($stage_generic_bound: ident$(, )?)+>)?$(($($input_name:ident$(,)?)+))? => $stage_condition:ident,)*
        }$(,)?
        default_stage: $last_stage_name:ident$(<$last_stage_generic:ident: $($last_stage_generic_bound: ident$(, )?)+>)?$(($($last_input_name:ident$(,)?)+))?
    ) => {
        use $crate::{
